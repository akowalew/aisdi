...
{
  BOOST_CHECK_EQUAL(map.size(), expected.size());
...
{
  BOOST_CHECK_EQUAL(OperationCountingObject::constructedObjects, count);
}

template <>
void thenDestroyedObjectsCountWas<OperationCountingObject>(std::size_t count)
{
  BOOST_CHECK_EQUAL(OperationCountingObject::destroyedObjects, count);
}

template <>
void thenCopiedObjectsCountWas<OperationCountingObject>(std::size_t count)
{
  BOOST_CHECK_EQUAL(OperationCountingObject::copiedObjects, count);
}

template <>
void thenMovedObjectsCountWas<OperationCountingObject>(std::size_t count)
{
  BOOST_CHECK_EQUAL(OperationCountingObject::movedObjects, count);
}

template <>
void thenAssignedObjectsCountWas<OperationCountingObject>(std::size_t count)
{
  BOOST_CHECK_EQUAL(OperationCountingObject::assignedObjects, count);
}
                              ...
                              TestedKeyTypes)
{
  ...
  BOOST_CHECK_THROW(map.begin()--, std::logic_error);
  BOOST_CHECK_THROW(--(map.begin()), std::logic_error);
  BOOST_CHECK_THROW(map.cbegin()--, std::logic_error);
  BOOST_CHECK_THROW(--(map.cbegin()), std::logic_error);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenMap_WhenInitializingFromListOfPairs_ThenAllItemsAreInMap,
                              ...
                              TestedKeyTypes)
{
  std::initializer_list<std::pair<const K, std::string>> ilist = { { 42, "Alice" }, { 27, "Bob" } };
  const Map<K> map = ilist;

  thenMapContainsItems(map, {ilist});
}
                              ...
                              TestedKeyTypes)
{
  ...
  BOOST_CHECK_THROW(map.at(1), std::logic_error);
}
                              ...
                              TestedKeyTypes)
{
  ...
  BOOST_CHECK_EQUAL(map.at(42), "Alice");
}
                              ...
                              TestedKeyTypes)
{
  ...
  BOOST_CHECK_THROW(map.erase(end(map)), std::logic_error);
}
